...
static void ToUpper(const char* str, size_t strLen, char* buffer,
                    size_t bufferSize)
{
    size_t i = 0;
    for (i = 0; i < strLen; i++)
...
int main(int argc, char** argv)
{
    ...
    {
        DefaultOutputPath(input.inputFilePath, strlen(input.inputFilePath),
                          &input.outputFilePath);
    }

    if (!input.arrayName)
    {
        DefaultArrayName(input.outputFilePath, strlen(input.outputFilePath),
                         &input.arrayName);
    }

    ArrayNameUpper(input.arrayName, strlen(input.arrayName),
                   &input.arrayNameUpper);
    ...
    {
        fprintf(outputStream, "alignas(%u) const unsigned char %s[] = {",
                input.alignment, input.arrayName);
    }
